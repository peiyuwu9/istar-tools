...
export async function getCustomerProposals(orderByCol, pageNum, limitNum) {
  const res = await fetch(
    `${apiUrl}/customer-proposals?orderByCol=${orderByCol}&pageNum=${pageNum}&limitNum=${limitNum}`
  );
...
export async function getCsutomers(orderByCol, pageNum, limitNum) {
  const res = await fetch(
    `${apiUrl}/customers?orderByCol=${orderByCol}&pageNum=${pageNum}&limitNum=${limitNum}`
  );
  return await res.json();
}
